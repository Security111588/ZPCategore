...
@implementation NSObject (ZPKeyValue)
#pragma mark - 公共方法
#pragma mark - 字典转模型
/**
 *  通过JSON数据来创建一个模型
...
+ (instancetype)objectWithKeyValues:(NSDictionary *)keyValues
{
    ...
    ZPAssert([keyValues isKindOfClass:[NSDictionary class]], desc);
...
+ (NSArray *)keyValuesArrayWithObjectArray:(NSArray *)objectArray
{
    ...
    ZPAssert([objectArray isKindOfClass:[NSArray class]], desc);
...
+ (NSArray *)objectArrayWithKeyValuesArray:(NSArray *)keyValuesArray
{
    ...
    ZPAssert([keyValuesArray isKindOfClass:[NSArray class]], desc);
...
{
    ZPAssertParamNotNil(propertyName);
    NSString *key = nil;
    // 1.查看有没有需要替换的key
